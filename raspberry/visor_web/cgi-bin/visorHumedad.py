...
def get_data(interval):
    ...
    if interval == None:
        curs.execute("SELECT * FROM hums")
    else:
        curs.execute("SELECT * FROM hums WHERE timestamp>datetime('now','-%s hours')" % interval)
...
def show_stats(option):
    ...
    curs.execute("SELECT timestamp,max(humedad) FROM hums WHERE timestamp>datetime('now','-%s hours')" % option)
    rowmax=curs.fetchone()
    rowstrmax="{0}&nbsp&nbsp&nbsp{1}C".format(str(rowmax[0]),str(rowmax[1]))

    curs.execute("SELECT timestamp,min(humedad) FROM hums WHERE timestamp>datetime('now','-%s hours')" % option)
    rowmin=curs.fetchone()
    rowstrmin="{0}&nbsp&nbsp&nbsp{1}C".format(str(rowmin[0]),str(rowmin[1]))

    curs.execute("SELECT avg(humedad) FROM hums WHERE timestamp>datetime('now','-%s hours')" % option)
    ...
    print "<h2>Humedad  Minima&nbsp</h2>"
    ...
    print "<div id='leyenda'>Rangos: Verde: &lt50 &nbsp Naranja: 50&ltx&lt80 &nbsp Rojo: &gt80 </div>"
...
def print_time_selector(option):
    ...
    print """<form action="/cgi-bin/visorHumedad.py" method="POST">
...
def main():
    ...
    printHTMLHead("Humedad LARUEX WorkStation", table)

    # print the page body
    print "<body>"
    print "<h1>Humedad LARUEX WorkStation <a id='ref_volver' href='main.py'><img id='icono_volver' src='http://158.49.250.145/images/back.png' alt='Volver'></a></h1>"
    print "<hr>"
